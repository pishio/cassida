...
export default function Modifiers(): React.JSX.Element {
  const copy = useT({
    ...
    ja: {
      ...
      nestingHeading: 'ネスト',
      nestingCopy:
        '修飾子は自由にネストできる。それぞれのコールバックは独自のスコープを開き、内側のチェーンは外側とは独立に LIFO で解決される。',
      sortHeading: 'デフォルトはモバイルファースト',
      sortCopy:
        'ソース順は問われない。幅基準のメディアクエリは小さい順 (min-width 昇順) に並べ替えられ、カスケードはその順番で読まれる。cassida.config.json でデスクトップファースト (max-width 降順) に切り替えられる。em / rem は 16px で正規化されるので、px と混ぜても相対順序が一貫する。幅基準でないクエリ (print、prefers-color-scheme など) は、幅ブロックの後にアルファベット順で並ぶ。',
    },
  });
