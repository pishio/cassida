...
export default function ExistingCss(): React.JSX.Element {
  const copy = useT({
    en: {
      ...
      unlayeredBody:
        'A declaration in no layer at all takes precedence over a declaration in any named layer. So plain legacy CSS — a stylesheet that never opted into `@layer` — overrides Cassida, not the other way round. To let Cassida win over it, pull that CSS into a layer too: import it with `layer()` and order it before `cas`. (`!important` reverses this: an `!important` declaration in an earlier layer beats a later one, so legacy `!important` rules are not solved by the layer trick above — `@layer cas` emits no `!important`.)',
    ...
    ja: {
      ...
      unlayeredBody:
        'どのレイヤーにも属さない宣言は、名前付きレイヤーのどの宣言よりも優先される。`@layer` を一度も使っていない素のレガシー CSS は、Cassida を上書きする。逆ではない。Cassida を勝たせたいなら、その CSS もレイヤーに入れる。`layer()` 付きで import し、`cas` より前に並べればよい。（`!important` はこれを反転させる。先のレイヤーの `!important` が後のレイヤーに勝つので、レガシー側が `!important` を使っている場合は上の手では勝てない。`@layer cas` は `!important` を出さない。）',
