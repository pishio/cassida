...
export default function Conditional(): React.JSX.Element {
  const copy = useT({
    en: {
      ...
      dynamicCopy:
        'v1 bailed when either branch carried a dynamic slot (CSS variable). v2 keeps the build-time path active: dynamic branches emit a parallel branch-conditional style={...} ternary that mirrors the className shape. Branches without dynamics use `void 0` so React skips style application cleanly.',
    ...
    ja: {
      ...
      dynamicCopy:
        'v1 はどちらかの分岐に動的スロット (CSS 変数) があるとビルド時パスを諦めていました。v2 はビルド時パスを維持し、動的な分岐に対して並列の分岐条件付き style={...} 三項式を className 三項式と同形で発行します。動的を持たない分岐は `void 0` を入れて React に style 適用をスキップさせます。',
