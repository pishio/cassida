...
**v0.11.0** — Phase 1 closing release. `@cassida/swc-plugin` / `@cassida/next-plugin` ABI policy is now documented + CI-enforced: the LTS-only Next.js support policy lives in the `@cassida/next-plugin` README, and a weekly GitHub Actions cron (`.github/workflows/swc-core-drift.yml`) compares `cassida_swc_plugin_next`'s `swc_core` pin against the latest Next.js 15 release, auto-opening a tracking issue when they diverge. Monorepo + `output: 'standalone'` guidance is documented (set `outputFileTracingRoot` to the monorepo root, not to the app directory, or `@cassida/*` is silently dropped from the standalone bundle's `node_modules`). The `.border(number)` / `.outline(number)` typecheck-runtime mismatch is fixed (the csstype generics widened the input union to `number`, but the runtime emitted invalid CSS — dropped the generic, numeric width now goes through `.borderWidth(1)` / `.outlineWidth(1)`). `tsconfig.typecheck.json` adoption is standardised across all 11 public packages with a CI guard. Docs site picks up a bilingual `PHILOSOPHY.md` + `/philosophy` page, a `/glossary` page (Single Class · Zero-runtime · LIFO Collapse · Bijection · canonical · longhand · shorthand-policy · modifier scope · `@layer cas` · `.props` terminator · cross-compiler bridge under stable anchors), a five-minute Install Quick Start, code-first ordering across plugin pages, plain-language Japanese rewrites across Landing / `cas() chain` / Property registry / Modifiers / Configuration / Unsafe / Recommended / Conditional / Install / Hover-fix / Global-css / Print, and a Frameworks section (Next.js setup, `@cassida/swc-plugin`, `@cassida/next-plugin`).
...
**v0.9.0** — `@cassida/next-plugin`'s multi-compiler architecture is reframed and hardened. v0.8.0's "documented multi-compiler race" was actually the structural cross-compiler bridge by which Server-only Server Component styles reach the Client compiler's `virtual.css` — per-compiler isolation would break Server-rendered `<aside className="cas-XXXXXXXX">` markup in the browser, because Next.js doesn't bundle pure Server Components into the Client graph. The store is now keyed `(compilerName, filename)` two-level (Next.js sets `'client'` / `'server'` / `'edge'` / `'middleware'`); `allRules()` merges every namespace on the read path — that merge IS the bridge. `CassidaWebpackPlugin` now taps `compiler.hooks.beforeRun` (production) and `compiler.hooks.watchRun` (dev) to clear only THAT compiler's namespace before its loaders re-run, so stale rules from since-deleted source files no longer accumulate in `next dev` while the bridge survives every compiler's independent lifecycle. The v0.8.0 `seen.length === 0` empty-store heads-up false-positived on legitimately empty fixtures and on Edge / Middleware compilers; it's replaced by a real race signature (peer compiler wrote at time T, my read at T+Δ sees zero rules from their namespace). The e2e fixture's post-build assert additionally verifies that Next.js actually injects `<link rel="stylesheet">` for the emitted `virtual.css` — catches the case where the import path or exports map regresses and Next.js silently drops the stylesheet despite a "successful" build.

**v0.8.0** — Next.js App Router support lands. `withCassida()` wraps `next.config.{js,mjs,ts}` to register `@cassida/swc-plugin` (a Rust-authored WASM transform on `experimental.swcPlugins`) and the matching `@cassida/next-plugin` IR-comment loader; `CassidaWebpackPlugin` exposes the aggregated `@layer cas` CSS via `import '@cassida/next-plugin/virtual.css'` from `app/layout.tsx`. The SWC plugin is shipped as two ABI-pinned WASM artefacts side by side (`wasmPathModern` for Rspack / @swc/core / plugin-react-swc, `wasmPathNext` for `@next/swc` 15.x) — the plugin ABI is version-bound to the host's `swc_core`, so the next-plugin transparently picks the right one. The whole chain is gated by a `consumer · next 15` end-to-end CI leg that runs `next build` + six post-build assertions (`@layer cas`, compiled `.cas-XXXXXXXX` selectors, rendered RSC class attributes, zero `cas(` runtime calls in client chunks, no placeholder leakage, client JS within budget). A documented Phase 1.x multi-compiler race remains; production builds emit a stderr heads-up when it actually fires, mutable via `CASSIDA_QUIET_RACE_WARNING=1`.
...
Earlier surface stays: `.props` terminator from v0.3 (separates chain methods from JSX prop typings — strict `tsc --noEmit` passes), cross-file static evaluation from v0.2 (design tokens defined in a separate module or `.json` file fold into static class hashes at build time), packaging hardening from v0.1.1 (`sideEffects: false`, pure-JS hasher, `exports` map). The feature set is covered by 482 unit tests (386 vitest across 11 npm packages + 96 Rust tests across the two SWC plugin crates), plus an end-to-end CI smoke test that builds a real consumer against the tarballs on Vite 5 / 6 / 7, Bun, and Next.js 15 — including `tsc --noEmit` against the consumer for every leg.
